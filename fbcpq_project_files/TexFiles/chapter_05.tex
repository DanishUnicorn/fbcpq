...
\begin{enumerate}
    ...
    A high fruit load increases the sink strength of fruits, enhancing assimilate export from leaves and stimulating photosynthesis. This “pull” effect prevents feedback inhibition, resulting in higher net photosynthetic rates; productive trees may photosynthesize at more than twice the rate of non-productive trees. However, despite increased activity, the overall assimilate reserve in the plant remains lower due to strong sink demand.
...
\begin{enumerate}
    ...
    Source strength describes the capacity of assimilatory organs, mainly leaves, to produce and export assimilates. It is defined as the product of \textit{source size} (leaf area) and \textit{source activity} (photosynthetic rate). Increased light or CO$_2$ levels enhance source activity, raising assimilate availability for growth and fruit development.
...
\begin{enumerate}
    \item \textbf{Causes of loss} 
        \begin{enumerate}
            \item \textit{Quantitative losses:} Mechanical stress, pest or disease damage, over-ripening, and water loss through transpiration.
            \item \textit{Qualitative losses:} Off-flavours, odours, and nutrient degradation due to metabolic activity in high-moisture fruits (80-90\%).
        \end{enumerate} 
    ...
    \item \textbf{Minimization strategies}  
        \begin{enumerate}
            \item \textit{Temperature control:} Store near 0\,\textdegree C (without freezing) to slow metabolism; pre-storage heating (30-40\,\textdegree C) prevents chilling injury.
            \item \textit{Atmosphere management:} Use Controlled or Modified Atmosphere (CA/MA) storage with low O\textsubscript{2}, high CO$_2$, and 1-MCP to reduce ethylene action and delay senescence.
            \item \textit{Humidity control:} Maintain around 90\% RH to prevent water loss.
            \item \textit{Harvest precision:} Harvest within the Final Harvest Window (FHW) to ensure storability and high post-storage quality.
        \end{enumerate}
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textit{Size and weight:} Strongly cultivar-dependent; large-fruited types are preferred for table use. Apple fruit weight, for instance, ranges from 80-200 g.  
