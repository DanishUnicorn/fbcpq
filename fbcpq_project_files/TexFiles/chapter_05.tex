...
\begin{enumerate}
    ...
    \subitem The \textit{number of plants per hectare} forms the base for yield potential and depends on the growing system and establishment of rooted runners \cite*{rm_01_L02_fruit_yield_quality}.
    \subitem \textit{Berry weight} is a major component that correlates linearly with the number of achenes per berry at a given density \cite*{rm_07.2_L11_13_fruit_growth_quality}.
    \subitem \textit{Flower quality} determines the number of ovule primordia and hence potential achene number, directly influencing final berry size \cite*{rm_07.1_L11_13_fruit_growth_quality}.
    \subitem \textit{Berry number per plant} depends on vegetative growth and spacing; higher inflorescence numbers may increase fruit count but reduce berry size due to competition \cite*{rm_07.1_L11_13_fruit_growth_quality}.
    
    \item \textbf{Sour cherry}
    \subitem The \textit{number and type of buds} are crucial yield components. Sour cherry has simple flower buds that contain only flowers or leaves, and flowering terminates growth at that site \cite*{rm_02_L03_buds_bud_development}. High yields can therefore reduce bud formation for the following season due to limited shoot growth \cite*{rm_03_L04_flower_bud_formation}.
    \subitem Each bud produces \textit{one to five flowers per cluster}, depending on cultivar \cite*{rm_07.1_L11_13_fruit_growth_quality}.
    \subitem \textit{Fruit size} is genetically limited by low sink activity, although minor increases occur when fruit numbers are reduced \cite*{rm_07.2_L11_13_fruit_growth_quality}.
    \subitem For processing cultivars, yield quality also depends on traits such as \textit{sugar content, acidity, ascorbic acid, anthocyanins, and aroma compounds}, which define fruit value for juice production \cite*{rmb_04_L21_postharvest_biology_technology_fruits_vegetables_flowers}.
\end{enumerate}

\subsection{Describe some important yield components in strawberry and in sour cherry}

%Yield in both crops is determined by fruit number and fruit size \cite*{rm_01_L02_fruit_yield_quality}. 

%In \textbf{strawberry}, yield depends on the number of plants per hectare, flower quality, berry number per plant, and berry weight. Flower quality defines achene number, which determines berry size, while increased berry number can reduce fruit size due to internal competition \cite*{rm_07.1_L11_13_fruit_growth_quality,rm_07.2_L11_13_fruit_growth_quality}.

%In \textbf{sour cherry}, yield is affected by bud number and type, flowers per bud, fruit size, and juice quality. Simple flower buds produce one to five flowers, and heavy yields can reduce shoot growth and bud formation for the next year. Fruit size is limited by low sink activity, and quality traits such as sugars, acids, and anthocyanins define value for processing \cite*{rm_02_L03_buds_bud_development,rm_03_L04_flower_bud_formation,rmb_04_L21_postharvest_biology_technology_fruits_vegetables_flowers}.
